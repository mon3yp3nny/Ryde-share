...
**Domain:** Multi-tenant API-first ride-coordination engine (GCP)
**Researched:** 2026-06-02
**Confidence:** HIGH (core stack), MEDIUM (frontend, anonymous token pattern)
...
```bash
# Core backend
npm install hono @hono/node-server @hono/zod-validator zod drizzle-orm pg jose

# GCP integrations
npm install @google-cloud/secret-manager

# Dev dependencies
npm install -D typescript tsx drizzle-kit vitest @types/node @types/pg eslint prettier
...
Cloud Run is the correct fit. The service is:
...
Set `--min-instances=1` on production to keep one warm instance. Set `--cpu-boost` to reduce cold starts when scaling beyond 1. CPU is allocated only during request processing (default); set `--cpu-always-on` only if needed for background cron (not in v1 scope).

GKE introduces cluster management overhead (node pools, upgrades, autoscaler tuning) with no upside for a single stateless API. App Engine Standard is reaching end-of-investment direction. Cloud Functions Gen2 is functionally Cloud Run under the hood but with a less ergonomic deployment model for a full REST API.

### Database: Cloud SQL PostgreSQL with RLS

**Multi-tenancy isolation pattern: shared schema + Row-Level Security (RLS)**

Use a single schema, single database. Every table gets a `org_id UUID NOT NULL` column. Drizzle schema enforces this at the type layer. PostgreSQL RLS policies filter all reads and restrict writes by `current_setting('app.current_org_id')`.

Set org context at the transaction level (not session level) to be safe with connection pooling:
```sql
BEGIN;
...
RLS adds ~2–4% overhead on indexed `org_id` columns — negligible for this workload.

Do NOT use schema-per-tenant: PostgreSQL degrades past ~100 schemas; running migrations per-tenant adds operational complexity that's disproportionate for this scale.

Do NOT use Firestore: the drive/seat/rider domain is deeply relational (FK integrity matters, coverage queries join across tables). Firestore's document model forces denormalization that hurts consistency guarantees.

**Cloud SQL tier:** Start with `db-g1-small` (shared-core, ~$9/month) for development. Move to `db-custom-2-7680` (2 vCPU / 7.5 GB) for production. High availability (regional failover replica) is a one-click option.

**Connection pooling:** Cloud SQL Managed Connection Pooling requires Enterprise Plus edition (~26% cost premium). For standard edition, use a PgBouncer sidecar in Cloud Run (transaction mode, max_server_connections tuned to Cloud SQL's max_connections). Alternatively, Drizzle's `pg` pool with `max: 5` per container instance is acceptable at low traffic because Cloud Run concurrency caps at 80 req/instance and scales horizontally.

### API Gateway: GCP API Gateway

Use GCP API Gateway as the front-door for rate limiting and API key management. The two classes of consumer are:

1. **Anonymous individuals** (join via link/QR): Hit the API directly through the gateway with no auth required. No API key. Rate limit by IP (basic DDoS mitigation via Cloud Armor if needed later). These routes are public.

2. **Authenticated API consumers** (handball-manager and future apps): Present an API key in `x-api-key` header. The gateway validates the key and enforces per-consumer quotas defined in the OpenAPI spec's `x-google-management` stanza (e.g., 1000 req/min per consumer).

The Hono backend enforces its own bearer-token check for routes that require organizer-level authorization within a drive (using the drive-scoped participation tokens described below). This is separate from gateway-level API key auth.

Upgrade path: if the platform grows to require advanced analytics, developer portal, or per-tenant monetization, migrate to Apigee X. The OpenAPI spec is portable.

### Anonymous Participation: Signed Drive Tokens

Anonymous users join a drive via a shareable link or QR code. The token model:

1. **Invite token embedded in the link**: A short-lived HMAC-signed token (`jose` HS256 JWT) with claims:
   - `sub`: drive ID
   - `org`: org ID  
   - `iat` / `exp`: issued-at and expiry (configurable, default 7 days for a drive link)
   - `role`: `participant`
   
   The token is URL-safe, stateless (no DB lookup to validate), and tamper-proof. Embed it as a path segment: `/join/{token}`.

2. **Session token issued on join**: When a user hits `/join/{token}`, the API mints a short-lived session JWT (24 h TTL) scoped to that `drive_id`. The browser stores this in `localStorage` or a cookie. All subsequent participant actions (claim seat, view coverage) attach this token. No password, no account.

3. **Organizer token**: The drive creator receives a separate organizer-scoped JWT with `role: organizer` at drive creation time. This token is stored by the creating consumer app (or shown once if standalone). It permits seat assignment, drive config edits, and coverage management.
...
The frontend is a standalone PWA that is a pure consumer of the same public API. It has no backend logic of its own — no server-side DB access, no separate API routes in the SvelteKit app.
...
Recommended for v1: static adapter. The ride coordination UI does not need SEO (it is accessed by link/QR, not discovered by search). Static export + Cloud CDN is the simplest deployment.
...
**If the handball-manager integration arrives:**
...
**If multi-stop trips are added later:**
...
**If traffic grows beyond Cloud Run's single instance:**
...
**If notifications/push are added (deferred):**
...
*Stack research for: ryde-share (multi-tenant GCP ride-coordination engine)*
*Researched: 2026-06-02*
